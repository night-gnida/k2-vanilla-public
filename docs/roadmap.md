...
2. Надёжность: собственный PLR для single-Z (kalico-модуль удалён,
   см. решение ниже); диагностика 485-шины;
...
## Решение по Kalico (2026-09-09): вырезан полностью

Kalico-хост и так не использовался (апстрим v0.13.0, аудит — ноль kalico-API).
Дожато: **power_loss_recovery.py удалён из комплекта** (kalico-only PLR, на
базе никогда не включался — секции в конфиге не было; install.sh теперь
подчищает старую копию из extras на принтере). Формулировки в README/конфигах
переписаны: не «вендоренные kalico-модули», а «открытые GPL-драйверы K2
(© Jacob10383), написанные для Kalico и аудированные на совместимость с
апстримом» — атрибуция сохранена по GPL. Драйверы железа остаются: ванильных
аналогов тензо-зонда/CFS/closed-loop не существует. PLR для single-Z теперь
означает «написать свой», а не «распорттировать каликовский».
